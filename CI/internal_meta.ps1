...
$urlJsonObsVersions = "https://s3.us-west-2.amazonaws.com/slobs-cdn.streamlabs.com/obsplugin/obsversions_internal.json"
...
foreach ($branch in $branchNames) {
    ...
    # Fetch the last 200 revisions for the branch
    $commits = git -C $repoPath log -200 --pretty=format:"%H %ct" | ForEach-Object {
        $parts = $_ -split " "
        @{
            sha = $parts[0]
